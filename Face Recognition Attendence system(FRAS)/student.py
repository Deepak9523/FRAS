...
# ************ bg image ******
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                ...
                my_cursor.execute("insert into my_student values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                                                self.var_dep.get(),
                                                                                                                self.var_course.get(),
                                                                                                                self.var_year.get(),
                                                                                                                self.var_sem.get(),
    ...
    def fetch_data(self):
         conn=mysql.connector.connect(host="localhost",username="root",password="root",database="face_detection")
         my_cursor=conn.cursor()
         my_cursor.execute("Select * from my_student")
        ...
        if self.var_dep.get()=="Select Department" or self.var_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                ...
                if update>0:
                    conn=mysql.connector.connect(host="localhost",username="root",password="root",database="face_detection")
                    my_cursor=conn.cursor()
                    my_cursor.execute("update my_student set Dep=%s,Course=%s,Year=%s,Sem=%s,Name=%s,Div=%s,Roll=%s,Gender=%s,Dob=%s,Email=%s,Phone=%s,Address=%s,Teacher=%s,Photo=%s where Std_id=%d",(
        ...
        if self.va_std_id.get()=="":
            ...
            try:
                ...
                if delete>0: 
                    conn=mysql.connector.connect(host="localhost",username="root",password="root",database="face_detection")
                    my_cursor=conn.cursor()
                    sql="delete from my_student where Std_id=%s"
    ...
    def genrate_dataset(self):
         if self.var_dep.get()=="Select Department" or self.var_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
               ...
               face_classifier=cv2.CascadeClassifier (r"C:\Users\DELL\Desktop\Face Recognition Attendence system(FRAS)\haarcascade_frontalface_default.xml")
